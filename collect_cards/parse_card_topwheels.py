...
def extract_model(header, card):
    brand = card.get('brand')
    if not brand:
        return ''
    
    brand_pos = header.find(brand)
    if brand_pos == -1:
        return ''
    
    sub_header = header[brand_pos + len(brand):].strip()
    
    words = sub_header.split()
    ...
    if brand == 'Wheelegend' and model == 'TCF' and len(words) > 1:
...
def extract_float_list(text: str) -> list:
    match = re.findall(r"[-+]?\d*\.?\d+", text.replace(',', '.'))
    return match if match else []

def parse_pcd(pcd_str: str):
    # Handles variations like '5x114.3' or '5х114.3' and multiple PCDs separated by '/'
    pcd_str = pcd_str.lower().replace('х', 'x')
    pcd_parts = pcd_str.split('/')
    pcd_data = []
    for part in pcd_parts:
        if 'x' in part:
            try:
                bolts, spacing = part.split('x', 1)
                bolts = int(bolts)
                spacing = extract_float(spacing)
                if bolts and spacing:
                    pcd_data.append({'bolts_count': bolts, 'bolts_spacing': spacing})
            except:
                continue
    return pcd_data if pcd_data else None
...
def parse_card_topwheels(soup: BeautifulSoup, brands_list:list, stop_list: list):
    ...
    try:
        ...
        # name
        if len(soup.select('span.inplace.auto-shy')) > 0:
            name = soup.select('span.inplace.auto-shy')[0].text
            logging.debug(name)
            card['name'] = name.replace('в Москве', '').replace('Диски', '').strip()                    
        ...
        # Extract additional parameters from the descriptive paragraph
        description_p = soup.find('p', class_='inplace mod__label_up_down auto-shy')
        if description_p:
            desc_text = description_p.get_text(separator='\n', strip=True)
            for line in desc_text.splitlines():
                line = line.strip().lower()
                if ':' in line:
                    label, value = map(str.strip, line.split(':', 1))
                    if 'диаметр цо' in label or 'dia' in label:
                        dia = extract_float_list(value)
                        if len(dia) == 2:
                            card['dia'] = dia[0]
                            card['dia_2'] = dia[1]
                        elif len(dia) == 1:
                            card['dia'] = dia[0]
                    elif 'диаметр' in label and 'цо' not in label and\
                        'dia' not in label:
                        card['diametr'] = extract_float(value)
                    elif 'ширина обода' in label or 'width' in label:
                        width = extract_float_list(value)
                        if len(width) == 2:
                            card['width'] = width[0]
                            card['width_2'] = width[1]
                        elif len(width) == 1:
                            card['width'] = width[0]
                    elif 'сверловка' in label or 'pcd' in label:
                        pcd_data = parse_pcd(value)
                        if pcd_data:
                            for i, pcd in enumerate(pcd_data, start=1):
                                if i == 1:
                                    card.update(pcd)
                                else:
                                    card[f'bolts_count_{i}'] = pcd['bolts_count']
                                    card[f'bolts_spacing_{i}'] = pcd['bolts_spacing']
                    elif 'вылет' in label or 'et' in label:
                        et = extract_float_list(value)
                        if len(et) == 2:
                            card['et'] = et[0]
                            card['et_2'] = et[1]
                        elif len(et) == 1:
                            card['et'] = et[0]
                    elif 'технология изготовления' in label:
                        if 'flow forming' in value:
                            card['rim_type'] = 'Литой'
        else:
            logging.debug('Description paragraph not found.')
        ...
        if not 'bolts_count' in card and not 'bolts_spacing' in card:
        ...
        rim_model = extract_model(header=name, card=card)
        if rim_model:
            card['rim_model'] = rim_model
        else:
            raise ElementNotFound('Element rim_model not found')
